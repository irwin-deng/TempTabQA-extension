def new_prompt(tables: list[str], questions: list[dict[str, dict[str, str|int]]],
...
Step 1. Elicit Information: Apply domain knowledge to understand how to approach and answer the question.

Step 2. Extract Relevant Rows: Identify and extract any rows from the table that could be relevant to the question. Explain why these rows are selected. Also output the relevant rows verbatim.

Step 3. Decompose the Question: Determine whether the original question can be solved by a simple table lookup. If not, break down the original question into between 2 and 4 smaller, more manageable sub-questions. Assume each sub-problem can be solved using the provided evidence. Explain how one could approach solving each sub-problem.
...
Step 5. Synthesize to Form the Final Answer: Using the answers to each sub-question, answer the original question. Explain the step-by-step reasoning process that leads to this final answer. Include any calculations or logical deduction to arrive at each conclusion, no matter how simple. Clearly and concisely state the final answer using "Final Answer:".
...
Step 5. Synthesize to Form the Final Answer
